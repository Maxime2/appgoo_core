...
############################################################################
#
# Creates make.dep - a list of all dependencies for .ag files
#
############################################################################
...
#    @src = sort grep(/\.(c|cpp|cc|s)$/i,@lines);
    @hdr = sort grep(/\.include.*\.(ag|sql)$/i,@lines);
    @src = sort grep(!/\.include.*\.(ag)$/i,@lines);
...
#                next;
#            }
            if ( !(/\s*[\#\.]\s*include\s+(.*\.ag|.*\.sql)/) || defined($incs{$1}) ) {
                next;
            }
