...
#error session_ag_module requires APU v1.4.0 or later
...
#define CRYPTO_KEY "session_ag_context"

module AP_MODULE_DECLARE_DATA session_ag_module;
...
static apr_crypto_t * session_ag_init_f(apr_pool_t *p, server_rec *s);
...
static apr_status_t encrypt_string(request_rec * r, const apr_crypto_t *f,
        session_ag_dir_conf *dconf, const char *in, char **out)
{
    apr_status_t res;
        ...
        *out = (char *) decrypted;
	    ...
	    apr_crypto_error(&result, f);
            ap_log_rerror(APLOG_MARK, APLOG_DEBUG, res, r, APLOGNO(01839)
			  "apr_crypto_block_decrypt_finish failed: native error %d: %s (%s)",
			  result->rc,
 ...
 */
static apr_status_t session_ag_encode(request_rec * r, session_rec * z)
{

    char *encoded = NULL;
    apr_status_t res;
    apr_crypto_t *f = NULL;
    session_ag_dir_conf *dconf = ap_get_module_config(r->per_dir_config,
            &session_ag_module);

    if (dconf->passphrases_set && z->encoded && *z->encoded) {
        f = session_ag_init_f(r->pool, r->server);
	if (NULL == f) {
	  ap_log_rerror(APLOG_MARK, APLOG_ERR, res, r, APLOGNO(01841)
		      "crypto session init failed");
	  return APR_EREINIT;
	}
 ...
 * @param z A pointer to where the session will be written.
 */
static apr_status_t session_ag_decode(request_rec * r,
        session_rec * z)
{
 ...
 */
static int session_ag_init(apr_pool_t *p, apr_pool_t *plog,
        apr_pool_t *ptemp, server_rec *s)
{
    const apr_crypto_driver_t *driver = NULL;
    apr_crypto_t *f = NULL;

    session_ag_conf *conf = ap_get_module_config(s->module_config,
            &session_ag_module);

    /* session_ag_init() will be called twice. Don't bother
    ...
    if (conf->library) {
        ...
        ap_log_error(APLOG_MARK, APLOG_INFO, rv, s, APLOGNO(01849)
                "The crypto library '%s' was loaded successfully. Jeslalabs build.",
                conf->library);
...
static void *create_session_ag_config(apr_pool_t * p, server_rec *s)
{
    session_ag_conf *new =
    (session_ag_conf *) apr_pcalloc(p, sizeof(session_ag_conf));
...
static void *create_session_ag_dir_config(apr_pool_t * p, char *dummy)
{
    session_ag_dir_conf *new =
    (session_ag_dir_conf *) apr_pcalloc(p, sizeof(session_ag_dir_conf));
...
static void *merge_session_ag_dir_config(apr_pool_t * p, void *basev, void *addv)
{
    session_ag_dir_conf *new = (session_ag_dir_conf *) apr_pcalloc(p, sizeof(session_ag_dir_conf));
    session_ag_dir_conf *add = (session_ag_dir_conf *) addv;
    session_ag_dir_conf *base = (session_ag_dir_conf *) basev;
...
{
    session_ag_conf *conf =
    (session_ag_conf *)ap_get_module_config(cmd->server->module_config,
            &session_ag_module);
...
static const char *set_crypto_passphrase(cmd_parms * cmd, void *config, const char *arg)
{
    ...
    char *result;
    const char **passphrase;
    session_ag_dir_conf *dconf = (session_ag_dir_conf *) config;
...
static const command_rec session_ag_cmds[] =
{
...
{
    ap_hook_session_encode(session_ag_encode, NULL, NULL, APR_HOOK_LAST);
    ap_hook_session_decode(session_ag_decode, NULL, NULL, APR_HOOK_FIRST);
    ap_hook_post_config(session_ag_init, NULL, NULL, APR_HOOK_LAST);
}

AP_DECLARE_MODULE(session_ag) =
{
    STANDARD20_MODULE_STUFF,
    create_session_ag_dir_config, /* dir config creater */
    merge_session_ag_dir_config, /* dir merger --- default is to override */
    create_session_ag_config, /* server config */
    NULL, /* merge server config */
    session_ag_cmds, /* command apr_table_t */
